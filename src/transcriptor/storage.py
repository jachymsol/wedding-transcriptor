...
class SegmentQueue:
    ...
    def enqueue(self, segment: TranscriptSegment) -> int:
        ...
        log.info(
            "Segment %d queued for retry (row %d)", segment.segment_id, row_id
        )
        return row_id

    def peek(self) -> list[tuple[int, TranscriptSegment]]:
        """Return all pending ``(row_id, segment)`` pairs in FIFO order.
        ...
        for row in rows:
            ...
            result.append((row["id"], TranscriptSegment.from_dict(data)))
        return result
